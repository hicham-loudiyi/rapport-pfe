...
\begin{document}
	...
	\begin{itemize}
		...
		Parmi les atouts majeurs d'un Agent AI, sa capacité à configurer dynamiquement le System Message (ou instruction système). Cette fonctionnalité permet de cadrer précisément le comportement du modèle, par exemple en définissant son rôle, et en contrôlant finement son style (ton, format, restrictions).
	...
	Le projet a abouti à la mise en place d’une API robuste et intuitive pour l’analyse technique d’erreurs, intégrant deux inputs, le premier étant un texte qui pourra être une copie d'une stacktrace ou bien simplement une description d'une erreur confrontée, et le deuxième - qui est optionnel - une image destinée à contenir une capture d’écran.
	
	Le recours à un modèle de langage multimodal s’est avéré donc nécessaire. À cet effet, le modèle \verb|Qwen2.5vl:7b| a été retenu pour ses capacités d’analyse conjointe du texte et de l’image.
	
	Quand au RAG, il a été utilisé pour améliorer la pertinence et la précision des analyses d’erreurs en enrichissant le texte et la capture d'écran avec des extraits pertinents du code source de l’application qui présente une erreur.
	
	\begin{figure}[H]
		\centering
		\fbox{\includegraphics{structure-projet.png}}
		\caption{\textit{Structure du projet}}
